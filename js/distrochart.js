...
var positions = [120, 285, 398, 540, 635];
var step = 40;
positions = positions.concat(d3.range(d3.max(positions)+step,1200,step))
step=30;
positions = positions.concat(d3.range(d3.max(positions)+step,5000,step))
var positions1 = [-100].concat(positions);
...
function makeDistroChart(settings) {
    ...
    function getBW(gName) {
        var bandWidthIndex = chart.orderedKeys.indexOf(gName);
        return diff(chart.xScale.range())[bandWidthIndex];
    }
    ...
    //     var bandWidth = getBW(gName)
    //     var width = bandWidth * (objWidth / 100);
    //     var padding = (bandWidth - width) / 2;
    //     var gShift = chart.xScale(gName);

    //     objSize.middle = bandWidth / 2 + gShift;
    ...
    function getObjWidth(objWidth, gName) {
        var objSize = { left: null, right: null, middle: null };
        var bandWidth = getBW(gName)
        var padding = (bandWidth) / 2;
        var gShift = chart.xScale(gName);

        objSize.middle = bandWidth / 2 + gShift;
        objSize.left = padding + gShift;
        objSize.right = objSize.left + bandWidth;
        return objSize;
    }
    /**
    ...
    function tooltipHover(groupName, metrics) {
        ...
        tooltipString += "<br\>Count: " + formatAsFloat(metrics.cnt);
    ...
    !function prepareSettings() {
        ...
        for (cName in chart.groupObjs) {
            ...
            var val = chart.groupObjs[cName].values.length+Math.random();
            unOrderGroups[val] = chart.groupObjs[cName]
            unOrderGroups[val]["cName"] = cName
        }
        chart.orderedKeys = []
        Object.keys(unOrderGroups).map(Number).sort(sortNumber).reverse().forEach(function (key) {
            chart.orderedKeys.push(unOrderGroups[key]["cName"])
        });
        ...
        chart.xScale = d3.scale.ordinal()
            .domain(chart.orderedKeys)
            .rangeBands([0, chart.width]);

        chart.xScale.range(positions);
    ...
    !function prepareChart() {
        ...
        // Create tooltip div
        chart.objs.tooltip = d3.select(".tooltip")
        for (var cName in chart.groupObjs) {
    ...
    chart.renderViolinPlot = function (options) {
        ...
        function calcNumBins(cName) {
            ...
            return 50 //Math.max(Math.round(2 * (iqr / Math.pow(chart.groupObjs[cName].values.length, 1 / 3))), 10)
        }
        ...
        chart.violinPlots.update = function () {
            ...
            for (cName in chart.groupObjs) {
                ...
                // console.log(cViolinPlot.histogramData);
